...
> **Status: historical design record, imported.** This is the first draft of
> the rover-control design, written before the Mission Platform epic existed
> and before this repository was split out. It is kept because the reasoning is
> the point, particularly the section auditing which premises of the original
> brief survived contact with the code, and the closing section on what the
> draft got wrong.
>
> Read it with three caveats. **The issue tracker supersedes it** wherever they
> disagree, including naming: the cockpit and the hardware-owning service have
> both been renamed since. **File and line references point into the private
> house repository**, not this one. **The Mission Platform epic supersedes
> Track C** entirely, because MCC v2's manifest-driven panels replace the idea
> of a separate bespoke cockpit application.
>
> What remains live and load-bearing: Track B, the service that owns the
> robot's hardware, and its two hard constraints. See
> [`docs/guide/helm.md`](../guide/helm.md).
