...
> **Status: historical design record, imported.** Brainstorm notes captured in
> the private house project before MERLE existed, from the era when the rover
> was still a plan and the wildlife detector was the only working layer. Kept
> for the reasoning, much of which became MERLE's shape: layered control with
> reflexes outranking thinking, the failsafe ladder, the dead-man timeout, the
> event grammar, and the choice of the Waveshare chassis as the ESP32/Pi split
> sold pre-built.
>
> Read it with three caveats. **The near-term milestone at the top is the house
